...
fq_poly_t *fq_poly_fast_exp(fq_poly_t *p, uint64_t power){
	uint64_t coeff[1] = {1};
	fp_poly_t *un_p = fp_poly_init(0,coeff, p->poly->carac);
	fq_poly_t *res = fq_poly_init(un_p, p->mod);
	fq_poly_t *p2 = fq_poly_init(p->poly, p->mod);
	fq_poly_t *tmp;
	while(power){ //ie while power!=0
		if( power & 1 ){ //power est impair
			tmp = fq_poly_mul(res, p2);//res = res * n;
			fq_poly_free(res);
			res = fq_poly_init(tmp->poly, tmp->mod);
			fq_poly_free(tmp);
		}
		tmp = fq_poly_mul(p2,p2);
		fq_poly_free(p2);
		p2 = fq_poly_init(tmp->poly, tmp->mod);
		fq_poly_free(tmp);
		power >>= 1;
	}
	fq_poly_free(p2);
